package hw6_21000506_nguyenthithanhhuong.ex03.time;
...
public class TimeSort {
    public static <T extends Comparable<T>> void timeSort(T[] array) {
        ...
        System.out.printf("Heap Sort: %fms\n", timeHeapSort(Arrays.copyOfRange(array, 0, array.length)));
    }
    ...
    // Heap Sort time in 20 times
    public static <T extends Comparable<T>> double timeHeapSort(T[] array) {
        List<Long> time = new ArrayList<>();

        for (int i = 0; i < 20; i++) {
            long temp = HeapSort.time(Arrays.copyOfRange(array, 0, array.length));
            time.add(temp);
        }
