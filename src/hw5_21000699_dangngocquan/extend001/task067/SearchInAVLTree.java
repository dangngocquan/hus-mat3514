package hw5_21000699_dangngocquan.extend001.task067;

import java.util.Scanner;

// Link: https://codelearn.io/learning/data-structure-and-algorithms?activityType=12&activityId=1312
public class SearchInAVLTree {
    public static void main(String[] args) {
        ...
        System.out.println(countValueInTree(root, x));
    }

    public static int countValueInTree(Node node, int x) {
        if (node == null) return 0;
        if (node.data == x) {
            return 1 + countValueInTree(node.left, x) + countValueInTree(node.right, x);
        } else if (x < node.data) {
            return countValueInTree(node.left, x);
        } else {
            return countValueInTree(node.right, x);
        }
    }
